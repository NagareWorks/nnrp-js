...
export function loadPackagedIpcBinding(): PackagedBindingResult {
  if (cachedBinding !== undefined) return cachedBinding;
  try {
    cachedBinding = { binding: hasDenoFfi() ? loadLazyDenoIpcBinding() : loadLazyNodeIpcBinding() };
  } catch (cause) {
    cachedBinding = {
      diagnostic: {
        code: "NNRP_IPC_NATIVE_ARTIFACT_UNAVAILABLE",
        message: cause instanceof Error ? cause.message : "IPC native artifact could not be loaded.",
        source: "transport",
        retryable: false,
        transport: "ipc",
        cause,
...
function loadLazyDenoIpcBinding(): NnrpNativeTransportBinding {
  ...
  if (!runtime.statSync(libraryPath).isFile) throw new Error(`IPC native library is not a file: ${libraryPath}`);
  let binding: NnrpNativeTransportBinding | undefined;
  const load = () => binding ??= loadDenoIpcBinding(runtime, libraryPath);
  return {
    mode: "deno-ffi",
...
function loadLazyNodeIpcBinding(): NnrpNativeTransportBinding {
  const nodeProcess = (globalThis as typeof globalThis & {
    process?: { release?: { name?: string } };
  }).process;
  if (nodeProcess?.release?.name !== "node") {
    throw new Error("IPC packaged native loading requires Node.js or Deno FFI.");
  }
  let binding: Promise<NnrpNativeTransportBinding> | undefined;
  const load = () => binding ??= import("./native-node.js").then((module) => module.loadNodeIpcBinding());
  return {
    mode: "managed-ffi",
...
function loadDenoIpcBinding(runtime: DenoRuntime, libraryPath: string): NnrpNativeTransportBinding {
  const library = runtime.dlopen(libraryPath, DENO_TRANSPORT_SYMBOLS);
  registerDenoRuntimeShutdown(library);
  return new DenoIpcBinding(library);
}
...
class DenoIpcBinding implements NnrpNativeTransportBinding {
...
class DenoIpcConnection implements NnrpTransportConnection {
  ...
  async receive(options: NnrpTransportReceiveOptions = {}): Promise<readonly Uint8Array[]> {
    ...
    try {
      if (owner.kind !== HANDLE_KIND_INVALID && owner.kind !== HANDLE_KIND_BUFFER) {
        throw transportError("NNRP_IPC_NATIVE_OWNER_INVALID", `Transport returned owner handle kind ${owner.kind}.`);
      }
  ...
  #requireOpen(): void {
    if (this.#closed) throw transportError("NNRP_IPC_CONNECTION_CLOSED", "IPC connection is closed.");
  }
}
...
function endpointBytes(endpoint: string | URL, operation: string): Uint8Array<ArrayBuffer> {
  ...
  if (value.length === 0) throw transportError("NNRP_IPC_ENDPOINT_INVALID", `IPC ${operation} endpoint is empty.`);
...
function decodePacketBatch(payload: Uint8Array, frameCount: number): readonly Uint8Array[] {
  ...
  for (let index = 0; index < frameCount; index++) {
    if (offset + 4 > payload.byteLength) throw transportError("NNRP_IPC_BATCH_INVALID", "Packet batch is truncated.");
    const length = new DataView(payload.buffer, payload.byteOffset + offset, 4).getUint32(0, true);
    offset += 4;
    if (offset + length > payload.byteLength) {
      throw transportError("NNRP_IPC_BATCH_INVALID", "Packet payload is truncated.");
    }
    packets.push(payload.slice(offset, offset + length));
    offset += length;
  }
  if (offset !== payload.byteLength) throw transportError("NNRP_IPC_BATCH_INVALID", "Packet batch has trailing bytes.");
  return packets;
}
...
function copyNativeBytes(address: bigint, lengthValue: bigint): Uint8Array {
  ...
  if (address === 0n || !Number.isSafeInteger(length) || length < 0) {
    throw transportError("NNRP_IPC_NATIVE_BUFFER_INVALID", "Transport returned an invalid native buffer view.");
  }
  const runtime = denoRuntime();
...
function decodeHandle(source: Uint8Array, expectedKind?: number): FfiHandle {
  if (source.byteLength < HANDLE_SIZE) {
    throw transportError("NNRP_IPC_NATIVE_HANDLE_INVALID", "Native handle is truncated.");
  }
  const view = dataView(source);
  const handle = {
    kind: view.getUint32(0, true),
  ...
  if (expectedKind !== undefined && (handle.kind !== expectedKind || handle.id === 0n || handle.generation === 0)) {
    throw transportError(
      "NNRP_IPC_NATIVE_HANDLE_INVALID",
