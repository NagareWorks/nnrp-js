...
Deno.test("@nnrp/browser-client suppresses cancelled payloads but preserves drop evidence", async () => {
  let polled = false;
  ...
  const session = runtime.connect({ endpoint: "wss://example.test/nnrp" }).openSession({ sessionId });
...
Deno.test("@nnrp/browser-client sends submit deadlines and protocol cancellation", async () => {
...
Deno.test("@nnrp/browser-client rejects pre-dispatch aborts and cleans terminal listeners", async () => {
  let submitCalls = 0;
  const controls: Array<{ messageType: NnrpMessageType; metadata: Record<string, unknown> }> = [];
  const runtime = await openBrowserRuntime({
    primitives: {
