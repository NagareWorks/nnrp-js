...
class DenoQuicBinding implements NnrpNativeTransportBinding {
  ...
  async probe(options: NnrpTransportProbeOptions): Promise<NnrpTransportProbeMetrics> {
    ...
    return await withSecurityConfig(this.library, options, "client", async (config) => {
  ...
  async listen(options: NnrpTransportEndpoint): Promise<NnrpTransportServer> {
    ...
    return await withSecurityConfig(this.library, options, "server", async (config) => {
      const output = bytes(HANDLE_SIZE);
      assertStatus(
...
function packOpenRequest(
  ...
  const view = dataView(request);
  view.setUint32(0, TRANSPORT_ID_QUIC, true);
...
async function withSecurityConfig<T>(
  library: DenoTransportLibrary,
  options: NnrpTransportEndpoint,
  mode: "client" | "server",
...
function createSecurityConfig(
  ...
  if (security === undefined || security.mode !== mode) {
    throw transportError("NNRP_QUIC_SECURITY_REQUIRED", `QUIC ${mode} requires ${mode} security configuration.`);
  }
  ...
  if (first.byteLength === 0 || second.byteLength === 0) {
    throw transportError("NNRP_QUIC_SECURITY_INVALID", `QUIC ${mode} security fields must be non-empty.`);
  }
  const request = bytes(40);
  const view = dataView(request);
  view.setUint32(0, TRANSPORT_ID_QUIC, true);
...
function assertStatus(status: Uint8Array, operation: string): void {
  if (status.byteLength < STATUS_SIZE) {
    throw transportError("NNRP_QUIC_FFI_STATUS_INVALID", `${operation} returned a short status.`);
  }
  const view = dataView(status);
  const code = view.getUint32(0, true);
  if (code !== 0) {
    throw transportError(
      "NNRP_QUIC_FFI_STATUS_ERROR",
...
function validateManifest(value: unknown, os: string, arch: string): string {
  if (typeof value !== "object" || value === null) throw new Error("QUIC native artifact manifest is not an object.");
  const manifest = value as Record<string, unknown>;
  if (manifest.package !== "nnrp-ffi-transport-quic" || manifest.transport_scope !== "quic") {
    throw new Error("QUIC package contains an artifact for another transport scope.");
  }
  if (manifest.abi_version !== ABI_VERSION) throw new Error(`QUIC artifact requires ABI ${ABI_VERSION}.`);
  if (manifest.os !== os || manifest.arch !== arch || manifest.library_kind !== "dynamic") {
    throw new Error(`QUIC artifact does not match ${os}/${arch} dynamic loading.`);
  }
  if (typeof manifest.library !== "string" || !/^[A-Za-z0-9_.-]+$/u.test(manifest.library)) {
    throw new Error("QUIC artifact manifest has an invalid library name.");
  }
  if (typeof manifest.source_archive_sha256 !== "string" || !/^[0-9a-f]{64}$/u.test(manifest.source_archive_sha256)) {
    throw new Error("QUIC artifact manifest has invalid checksum metadata.");
  }
  ...
  if (missing.length > 0) throw new Error(`QUIC artifact is missing transport exports: ${missing.join(", ")}.`);
...
function nativePlatform(
  ...
  if (!(["windows", "macos", "linux"] as const).includes(os as "windows" | "macos" | "linux")) {
    throw new Error(`QUIC Deno FFI does not support ${osValue}.`);
  }
  if (!(["x86_64", "x86", "aarch64", "armv7"] as const).includes(arch as "x86_64" | "x86" | "aarch64" | "armv7")) {
    throw new Error(`QUIC Deno FFI does not support ${archValue}.`);
  }
...
function denoRuntime(): DenoRuntime {
  ...
  if (runtime === undefined || typeof runtime.dlopen !== "function") {
    throw new Error("QUIC packaged native loading requires Deno FFI or an explicit managed binding.");
  }
  return runtime;
}
...
function transportError(code: string, message: string, retryable = false): NnrpTransportError {
  return new NnrpTransportError({ code, message, source: "transport", retryable, transport: "quic" });
}
