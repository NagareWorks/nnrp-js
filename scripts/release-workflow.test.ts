...
Deno.test("release workflow and local preparation pin Rust preview4.4", () => {
  assertEquals(releaseWorkflow.match(/1\.0\.0-preview\.4\.4/g)?.length, 3);
  assertStringIncludes(artifactPreparation, 'DEFAULT_RUST_ARTIFACT_VERSION = "1.0.0-preview.4.4"');
  assertEquals(releaseWorkflow.includes("1.0.0-preview.4.3"), false);
  assertEquals(artifactPreparation.includes("1.0.0-preview.4.3"), false);
});
